...
public protocol TextViewCoordinator: AnyObject {
    ...
    /// Called when the text view requests a context menu.
    /// - Parameters:
    ///   - event: The event that triggered the menu request (usually right click).
    ///   - controller: The text controller.
    /// - Returns: An optional menu to display. If multiple coordinators return a menu, the first non-nil one is used.
    @MainActor
...
public extension TextViewCoordinator {
    ...
    func textViewDidChangeText(controller: TextViewController) { }
    func textViewDidChangeSelection(controller: TextViewController, newPositions: [CursorPosition]) { }
    func menu(for event: NSEvent, in controller: TextViewController) -> NSMenu? { nil }
    func destroy() { }
}
