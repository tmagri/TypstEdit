...
class SuggestionViewController: NSViewController {
    ...
    func moveSelectionUp() {
        ...
        print("[SuggestionViewController] moveSelectionUp - New Row: \(newRow)")
        tableView.selectRowIndexes(IndexSet(integer: newRow), byExtendingSelection: false)
        tableView.scrollRowToVisible(newRow)
    }

    func moveSelectionDown() {
        let newRow = min((model?.items.count ?? 1) - 1, tableView.selectedRow + 1)
        print("[SuggestionViewController] moveSelectionDown - New Row: \(newRow)")
        tableView.selectRowIndexes(IndexSet(integer: newRow), byExtendingSelection: false)
        tableView.scrollRowToVisible(newRow)
    }
...
extension SuggestionViewController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    public func tableView(_ tableView: NSTableView, rowViewForRow row: Int) -> NSTableRowView? {
        CodeSuggestionRowView { [weak self] in
            self?.model?.activeTextView?.theme.selection ?? NSColor.selectedTextBackgroundColor
        }
    }
    ...
    public func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        // Redraw all visible rows to update isSelected state
